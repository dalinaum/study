...
func GetAllMessages(ctx context.Context, db *db.MongoCollections, sender string, receiver string) (*pb.GetAllMessageResponse, error) {
	var messages []*pb.Message
	// check if receiver and sender are the same or not
	if strings.EqualFold(sender, receiver) {
		return nil, status.Errorf(codes.InvalidArgument, "sender and receiver must not be same")
	}
